...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        setMapStyle()
        viewModel.maps.observe(this) { result ->
            when (result) {
                is Resource.Success -> {
                ...
                is Resource.Error -> {
                    Log.e("MapActivity", result.error)
                    Snackbar.make(binding.root, "Terjadi Kesalahan", Snackbar.LENGTH_SHORT).show()
                }

                is Resource.Loading -> {
                    Snackbar.make(binding.root, "memuat..", Snackbar.LENGTH_SHORT).show()
                }
            }
        }
    ...
    private fun handleData(data: List<ListStoryItem>) {
        data.forEach { item ->
            lat = item.lat
            lon = item.lon
